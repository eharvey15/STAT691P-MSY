...
--------------
-------------------------------------------
## DATA SIMULATION VARIABLES (MODEL 2) ----  
-------------------------------------------
...
----------------------------------------------
## DATA SIMULATION COEFFICIENTS (MODEL 2) ----
----------------------------------------------
...
fit.logistic.2 <- glm(y ~ x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8 + x9 + x10 + x11 + x12 - 1, data=train.batch, family=binomial("logit"))
...
test_prob2 <- predict(fit.logistic.2, newdata = test.batch, type = "response")
# create ROC curve information and AUC

test_roc2 <- roc(test.batch$y ~ test_prob2, plot = FALSE, print.auc = TRUE)

as.numeric(test_roc2$auc)

# plot the ROC curve and show AUC on curve

plot(test_roc2, main="ROC Curve for Test Batch \n Logistic Regression 2", print.auc=TRUE)



------------------------------------
## WRITE SIMULATED MODEL 2 DATA ----
------------------------------------
